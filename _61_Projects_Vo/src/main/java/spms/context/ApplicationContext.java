...
public class ApplicationContext {
	...
	// properties파일이 key = value 구조로 되어 있기 때문에 Map을 사용함
	...
	// properties 파일을 읽어서 객체를 참조하거나 생성함
	private void prepareObjects(Properties props) throws Exception {
		...
		for(Object item : props.keySet()) {
			...
			// Tomcat의 DataSource는 객체를 찾아서 참조값을 얻음
			...
			// 우리가 직접 new로 객체를 생성
				...
				// 클래스 정보를 바탕으로 객체를 동적 생성해서 objTable에 key와 value(객체)를 넣음
	...
	// Controller 상속받은 객체는 MemberDao 객체를 주입
	private void injectDependency() throws Exception {
		for(String key : objTable.keySet()) {
			
			// DataSource 객체를 제외한 나머지는 모두 외부에서 객체를 주입해야 할 setter가 있음
			if(!key.startsWith("jndi.")) {
	...
	private void callSetter(Object obj) throws Exception {
		...
		// 객체를 통해 클래스 정보를 얻고 클래스 정보로 모든 메서드 정보를 가져옴
		// getMethods : reflection 기능
		Method[] ms = obj.getClass().getMethods();
		for(Method m : ms) {
			// setter 메서드인가?
			if(m.getName().startsWith("set")) {
				
				// setter 메서드의 parameter는 무조건 하나라서 이렇게 코드를 작성
				/// setter 메서드의 첫 번째 파라미터(즉, 주입해야 할 객체 타입) 정보
				dependency = findObjectByType(m.getParameterTypes()[0]);
				
				// 해당 파라미터에 해당하는 객체를 찾아서 반환 받았으면,
				// setter를 호출하고, 첫 번째 파라미터에 객체를 넣음
				if(dependency != null) {
