...
SELECT TO_CHAR(sysdate, '"오늘은 " YYYY"년 " MM "월 " DD"일입니다."') 
FROM dual;
...
-- 소수점 기준으로 앞은 여섯 자리, 소수점 기준으로 뒤는 다섯 자리까지 출력
SELECT TO_CHAR(12345.678, '999,999.99999') num 
FROM dual;

-- 여섯 자리가 다 채워지지 못하면 앞을 0으로 채워라
...
-- 원래 범위보다 작으면 출력장애가 발생해 ########로 출력됨
SELECT TO_CHAR(12345.678, '9,999.9') num 
FROM dual;

-- 소숫점 윗자리만 개수가 잘 맞으면 됨
-- 소숫점 아래자리는 알아서 반올림이 되므로 오류가 나지 않음
SELECT TO_CHAR(2345.6789012345, '9,999.9') num 
FROM dual;
