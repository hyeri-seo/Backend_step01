...
SELECT sal*12+NVL(comm, 0) 연봉, eno 사원번호, ename 사원명  
FROM emp 
...
-- null 값은 계산의 대상이 아니기 때문에 자동 제외됨
SELECT comm 보너스, eno 사원번호, ename 사원명 
FROM emp 
WHERE comm <= 600;

-- null 값을 0으로 치환해서 계산한다는 규정이 있다면 아래처럼
SELECT comm 보너스, eno 사원번호, ename 사원명 
FROM emp 
