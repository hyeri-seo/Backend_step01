...
SELECT ename, SUBSTR(ename, 2),         -- 두 번째 글자부터
              SUBSTR(ename, -2),        -- 뒤에서 두 번째 글자부터
...
SELECT INSTR('database', 'a'),          -- 처음부터
       INSTR('database', 'a', 3),       -- 세 번째 글자 이후
       INSTR('database', 'a', 1, 3)     -- 첫 번째 글자 이후 a가 세 번째 보이는 위치
FROM dual;
...
SELECT TRIM('남' from '남기남'),                -- 전부
        TRIM(leading '남' from '남기남'),       -- 앞에서
        TRIM(trailing '남' from '남기남'),      -- 뒤에서
        TRIM('남' from '남남남기남남남')        -- 전부
FROM dual;
...
SELECT TRANSLATE('World of Warcraft', 'Wo', '-*') TRANSLATE,    -- W는 -로, o는 *(글자 각각)
        REPLACE('World of Warcraft', 'Wo', '--') REPLACE        -- Wo라는 단어를 --로 바꾸기(단어 통째로)
FROM dual;
