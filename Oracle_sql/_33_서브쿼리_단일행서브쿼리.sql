...
3) 김연아와 부서가 다르고 동일한 업무를 하는 사원의 정보를 검색하라
 ...
 SELECT dno FROM emp WHERE ename = '김연아';

 b) 김연아의 업무를 검색한다 - Sub Query
SELECT job FROM emp WHERE ename = '김연아';

 c) 위 조건의 사원의 정보를 검색한다 - Main Query
SELECT dno 부서번호, job 업무, eno 사번, ename 이름 
FROM emp 
...
--부서별 평균급여
SELECT dno 부서번호, AVG(sal) "부서별 평균급여"
FROM emp 
GROUP BY dno;

-- 부서별 평균을 구하고 가장 큰 값을 구함
SELECT MAX(AVG(sal)) "부서별 평균급여"
FROM emp 
GROUP BY dno;
